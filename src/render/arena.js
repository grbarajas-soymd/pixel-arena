...
import { CW, CH, AX, AY, AW, AH, GY, GY_MIN, GY_MAX, TK } from '../constants.js';
...
export function initGround(){
  ...
  for(let gx=AX;gx<AX+AW;gx+=40){
    for(let gy=GY_MIN-20;gy<GY_MAX+40;gy+=20){
      const r=Math.random();
...
function spawnAmbient(b){
  ...
  if(b.ambient==='ember')state.ambientParticles.push({x:x,y:GY+Math.random()*20,vx:(Math.random()-.5)*8,vy:-Math.random()*15-5,life:2+Math.random()*2,maxLife:4,r:1.5+Math.random(),col:Math.random()>.5?'#aa4a1a':'#c87a3a',type:'float'});
  else if(b.ambient==='snowflake')state.ambientParticles.push({x:x,y:AY-10,vx:(Math.random()-.5)*10,vy:Math.random()*12+5,life:4+Math.random()*3,maxLife:7,r:1+Math.random(),col:'#8a9aaa',type:'float'});
  else if(b.ambient==='wisp')state.ambientParticles.push({x:x,y:y,vx:Math.sin(Math.random()*6.28)*6,vy:Math.cos(Math.random()*6.28)*4,life:3+Math.random()*3,maxLife:6,r:2+Math.random()*2,col:Math.random()>.5?'#5a4488':'#6a5498',type:'wisp',phase:Math.random()*6.28});
  else if(b.ambient==='firefly')state.ambientParticles.push({x:x,y:y,vx:(Math.random()-.5)*5,vy:(Math.random()-.5)*3,life:4+Math.random()*3,maxLife:7,r:1.5,col:'#5a8a2a',type:'wisp',phase:Math.random()*6.28});
  else if(b.ambient==='ash')state.ambientParticles.push({x:x,y:AY-10,vx:(Math.random()-.5)*8,vy:Math.random()*8+3,life:3+Math.random()*2,maxLife:5,r:1+Math.random()*0.5,col:'#4a4440',type:'float'});
}
...
export function render(){
  ...
  if(state.bt<3000){var bAlpha=state.bt<1000?state.bt/1000:Math.max(0,(3000-state.bt)/2000);ctx.globalAlpha=bAlpha*0.4;ctx.fillStyle='#000';ctx.font='bold 10px "Cinzel"';ctx.textAlign='center';ctx.fillText(b.name,CW/2+1,AY-12+1);ctx.fillStyle=b.fogCol?'#ccbbaa':'#8a8a8a';ctx.fillText(b.name,CW/2,AY-12);ctx.globalAlpha=1;}
  const {h1,h2}=state;
  ...
  // Depth-sorted rendering: collect all entities, sort by y (back to front)
  if(h1&&h2){
    var entities=[];
    for(const h of[h1,h2]){
      entities.push({type:'hero',obj:h,y:h.y||GY});
      if(h.follower&&h.follower.alive)entities.push({type:'follower',obj:h.follower,owner:h,y:h.follower.y||GY});
      if(h.arenaFollowers)for(var afi=0;afi<h.arenaFollowers.length;afi++){var af=h.arenaFollowers[afi];if(af.alive)entities.push({type:'arenaFollower',obj:af,y:af.y||GY})}
    }
    entities.sort(function(a,b){return a.y-b.y});
    for(var ei=0;ei<entities.length;ei++){
      var ent=entities[ei];
      if(ent.type==='hero')drawHero(ent.obj);
      else if(ent.type==='follower')drawFollower(ent.obj,ent.owner);
      else if(ent.type==='arenaFollower')drawArenaFollower(ent.obj);
    }
  }
  ...
  for(const f of state.floats){ctx.globalAlpha=f.life/f.maxLife;ctx.fillStyle='#000';ctx.font='bold 13px "Cinzel"';ctx.textAlign='center';ctx.fillText(f.text,f.x+1,f.y+1);ctx.fillStyle=f.color;ctx.fillText(f.text,f.x,f.y);ctx.globalAlpha=1;}
  if(h1&&h2){ctx.fillStyle='rgba(90,70,40,0.6)';ctx.font='bold 9px "Cinzel"';ctx.textAlign='center';ctx.fillText(Math.round(dst(h1,h2))+'u',CW/2,AY-5)}
